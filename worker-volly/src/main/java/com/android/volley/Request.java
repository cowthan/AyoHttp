...
public abstract class Request<T> implements Comparable<Request<T>> {
    ...
    /**
     * URL of the 301/302 redirect. It will be populated by the network layer
     * upon redirection
     */
    ...
    /**
     * Params to be added to the request body in case of a POST or PUT request
     * or appended to the URL in case of a GET request
     */
    ...
    protected OnCacheListener<T> cacheListener;

    protected OnNotModifyListener notModifyListener;
    ...
    /**
     * Creates a new request with the given method (one of the values from
     * {@link Method}), URL, priority, error listener and retry policy. Note
     * that the normal response listener is not provided here as delivery of
     * responses is provided by subclasses, who have a better idea of how to
     * deliver an already-parsed response.
     */
    public Request(int method, String url, Priority priority, Response.ErrorListener listener,
        ...
        if (MarkerLog.ENABLED) {
            try {
                ...
                if (Looper.myLooper() != Looper.getMainLooper()) {
                    ...
                    // actually do it on the main thread to ensure correct
                    // ordering.
    ...
    /**
     * Returns the URL of this request. In case of redirection return the
     * redirect URL
     */
    ...
    public Map<String, String> getHeaders() throws AuthFailureError {
        ...
        return ((mRequestHeaders == null) ? Collections.<String, String> emptyMap()
                : mRequestHeaders);
    }
    ...
    /**
     * Sets the parameters to be added to the request body in case of a POST or
     * PUT request, or appended to the URL in case of GET request
     *
     * @param params
     */
    ...
    protected void deliverCacheResponse(T response) {
        if (null != cacheListener) {
            cacheListener.onResponse(response);
        }
    }

    protected void deliverNotModify() {
        if (notModifyListener != null) {
            notModifyListener.onResponse();
        }
    }
    ...
    public void setNeedCacheFirst(boolean needCacheFirst) {
        this.needCacheFirst = needCacheFirst;
    }

    public boolean isNeedCacheFirst() {
        return needCacheFirst;
    }
