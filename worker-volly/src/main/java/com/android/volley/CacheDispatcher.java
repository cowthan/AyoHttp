...
public class CacheDispatcher extends Thread {
    ...
    public void run() {
        ...
        while (true) {
            try {
                // release previous request object to avoid leaking request
                // object
                request = null;
                ...
                if (entry.isExpired() && !request.isNeedCacheFirst()) {
                    request.addMarker("cache-hit-expired");
                    request.setCacheEntry(entry);
                    mNetworkQueue.put(request);
                    continue;
                }
                if (!request.isNeedCacheFirst()) {
                    request.addMarker("is-not-need-cache-first");
                    request.setCacheEntry(entry);
                    mNetworkQueue.put(request);
                    continue;
                }
                ...
                // network.
                if (request.isNeedCacheFirst()) {
                    request.addMarker("response-cache-first");
