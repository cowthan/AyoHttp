...
public abstract class DownLoadSubscribe extends Subscriber<ResponseBody> {
    ...
    public DownLoadSubscribe(@NonNull String fileName) {
        mSaveFilePath = new File(Environment.getExternalStorageDirectory(), "okhttp-cache").getAbsolutePath();//FileUtils.getCacheDir(SolidApplication.getInstance()).getAbsolutePath();
